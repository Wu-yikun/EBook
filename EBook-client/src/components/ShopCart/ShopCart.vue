<template>
  <div v-if="$route.path != '/login'">
        ...
        <div class="bg-white shopcart border-right border-secondary">
          <div v-show="books.length">
              ...
              <li v-for="(book, index) in books" :key="index" class="list-unstyled mb-2 mt-1 ">
                <div class="row mt-3">
                  ...
                  <div class="col-md-6">
                    ...
                    <div class="col-md-12">
                      <i class="iconfont icon-jian" @click="substract(book.count, index)" :style="book.count>1? 'cursor: pointer':''" :class="book.count>1? 'text-danger':''"></i>
                      ×{{ book.count }}
                      <i class="iconfont icon-jia" @click="add(book.count,book.stock, index)" :style="book.count<book.stock? 'cursor: pointer':''" :class="book.count<book.stock? 'text-danger':''"></i>
                    </div>
                  </div>
            ...
            <div class="mt-3 mb-3 row" v-show="total.total_money!=0">
              ...
              <el-button class="clearBtn" @click="clearShopCart">清空购物车</el-button>
              <el-button type="danger" class="submitBtn" @click="submitOrder">
                提交订单
              </el-button>
            </div>
          </div>
    ...
    <transition name="shopcart">
      <div class="bg-success float wrap" style="cursor: pointer" @click="showShopCart = !showShopCart" v-show="showShopCart">
    ...
    <transition name="float">
      <div class="shopcart-float bg-success wrap" style="cursor: pointer" @click="showShopCart = !showShopCart" v-show="!showShopCart">
        <el-badge :value="total.total_books" class="center" :hidden="total.total_books === 0">
          <i class="iconfont icon-gouwuche"></i>
        </el-badge>
      </div>
